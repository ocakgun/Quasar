...
if(BTB_ENABLE){
  ...
  io.dec_i0_branch_d     := i0_dp.condbr | i0_dp.jal | i0_br_error_all
...
//  io.dec_div_active := withClock(io.free_l2clk){RegNext(div_active_in, 0.U)}
  ...
  io.dec_i0_result_r         :=  i0_result_r
...
object dec_decode extends App {
	(new chisel3.stage.ChiselStage).emitVerilog(new dec_decode_ctl())
}