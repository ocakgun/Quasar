...
class el2_dec_trigger extends Module with el2_lib {
  val io = IO(new Bundle {
    ...
    val dec_i0_pc_d = Input(UInt(32.W))
    val dec_i0_trigger_match_d = Output(UInt(4.W))
  })
  val dec_i0_match_data = VecInit.tabulate(4)(i =>  repl(32, (!io.trigger_pkt_any(i).select & io.trigger_pkt_any(i).execute)) & Cat(io.dec_i0_pc_d(31,1), io.trigger_pkt_any(i).tdata2(0)))
