...
class dma_ctrl extends Module with lib with RequireAsyncReset {
  val io = IO(new Bundle {
    ...
    val dbg_cmd_type          = Input(UInt(2.W))
    val dbg_cmd_size          = Input(UInt(2.W))

    val dbg_dma_bubble        = Input(Bool())
    val dma_dbg_ready         = Output(Bool())
    val dma_dbg_cmd_done      = Output(Bool())
    val dma_dbg_cmd_fail      = Output(Bool())
    val dma_dbg_rddata        = Output(UInt(32.W))
    val dma_dccm_req          = Output(Bool())
    val dma_iccm_req          = Output(Bool())
    val dma_mem_tag           = Output(UInt(3.W))
    val dma_mem_addr          = Output(UInt(32.W))
    val dma_mem_sz            = Output(UInt(3.W))
    val dma_mem_write         = Output(Bool())
    val dma_mem_wdata         = Output(UInt(64.W))
    val dccm_dma_rvalid       = Input(Bool())
    val dccm_dma_ecc_error    = Input(Bool())
    val dccm_dma_rtag         = Input(UInt(3.W))
    val dccm_dma_rdata        = Input(UInt(64.W))
    ...
    val dma_dccm_stall_any    = Output(Bool())
    val dma_iccm_stall_any    = Output(Bool())
    val dccm_ready            = Input(Bool())
    val iccm_ready            = Input(Bool())
    val dec_tlu_dma_qos_prty  = Input(UInt(3.W))
  ...
  val fifo_addr_in = Mux(io.dbg_cmd_valid, io.dbg_cmd_addr, bus_cmd_addr)
  val fifo_byteen_in = Fill(8,!io.dbg_cmd_valid) & bus_cmd_byteen    // Byte enable is used only for bus requests//Mux(io.dbg_cmd_valid, 0.U, bus_cmd_byteen)
  val fifo_sz_in = Mux(io.dbg_cmd_valid, Cat(0.U,io.dbg_cmd_size), bus_cmd_sz)
  val fifo_write_in = Mux(io.dbg_cmd_valid, io.dbg_cmd_write, bus_cmd_write)
...
val fifo_cmd_en = WireInit(UInt(DMA_BUF_DEPTH.W), 0.U)
  ...
  fifo_data_en      := (0 until DMA_BUF_DEPTH).map(i => (((bus_cmd_sent & fifo_write_in & io.dma_bus_clk_en) | (io.dbg_cmd_valid & io.dbg_cmd_type(1) & io.dbg_cmd_write)) & (i.U === WrPtr).asUInt()) | ((dma_address_error | dma_alignment_error) & (i.U === RdPtr).asUInt()) | (io.dccm_dma_rvalid & (i.U === io.dccm_dma_rtag).asUInt()) | (io.iccm_dma_rvalid & (i.U === io.iccm_dma_rtag).asUInt())).reverse.reduce(Cat(_,_))
  ...
  val fifo_data_in = VecInit.tabulate(DMA_BUF_DEPTH)(i =>(Mux(fifo_error_en(i) & (fifo_error_in(i).orR), Cat(Fill(32, 0.U), fifo_addr(i)), Mux(io.dccm_dma_rvalid & (i.U === io.dccm_dma_rtag), io.dccm_dma_rdata, Mux(io.iccm_dma_rvalid & (i.U === io.iccm_dma_rtag), io.iccm_dma_rdata, Mux(io.dbg_cmd_valid, Fill(2, dma_dbg_mem_wrdata), bus_cmd_wdata(63,0)))))))
  ...
  val RdPtrEn = io.dma_dccm_req | io.dma_iccm_req | (dma_address_error | dma_alignment_error | dma_dbg_cmd_error)
  ...
  //Dbg outputs
  io.dma_dbg_ready    := fifo_empty & io.dbg_dma_bubble
  ...
  dma_dbg_cmd_error := fifo_valid(RdPtr) & ~fifo_done(RdPtr) & fifo_dbg(RdPtr) &
   ...
   dma_dbg_mem_wrdata := Mux1H(Seq(
     (io.dbg_cmd_size(1,0) === "h0".U(2.W)) -> Fill(4,io.dbg_cmd_wrdata(7,0)) ,
     (io.dbg_cmd_size(1,0) === "h1".U(2.W)) -> Fill(2,io.dbg_cmd_wrdata(15,0)),
     (io.dbg_cmd_size(1,0) === "h2".U(2.W)) -> io.dbg_cmd_wrdata ))
  ...
  // Nack counter, stall the lsu pipe if 7 nacks
   ...
   dma_nack_count_d   := Mux((dma_nack_count >= dma_nack_count_csr), (Fill(3,(!(io.dma_dccm_req | io.dma_iccm_req))) & dma_nack_count),
                        Mux((dma_mem_req & ~(io.dma_dccm_req | io.dma_iccm_req)), (dma_nack_count + 1.U), 0.U(3.W)))
  ...
  io.dma_mem_addr      := Mux((io.dma_mem_write & ~fifo_dbg(RdPtr) & (dma_mem_byteen === "hf0".U(8.W))), Cat(dma_mem_addr_int(31,3),1.U,dma_mem_addr_int(1,0)), dma_mem_addr_int)
  io.dma_mem_sz        := Mux(io.dma_mem_write & ~fifo_dbg(RdPtr) & ((dma_mem_byteen === "h0f".U(8.W)) | (dma_mem_byteen === "hf0".U(8.W))), 2.U(3.W), dma_mem_sz_int)
  ...
  dbg_dma_bubble_bus := rvdff_fpga(io.dbg_dma_bubble,dma_bus_clk,io.dma_bus_clk_en,clock)
  ...
  val wrbuf_tag = rvdffs_fpga(io.dma_axi_awid,wrbuf_en,dma_bus_clk,io.dma_bus_clk_en,clock)
  val wrbuf_sz = rvdffs_fpga(io.dma_axi_awsize,wrbuf_en,dma_bus_clk,io.dma_bus_clk_en,clock)
  ...
  val wrbuf_byteen = rvdffs_fpga(io.dma_axi_wstrb,wrbuf_data_en,dma_bus_clk,io.dma_bus_clk_en,clock)
  ...
  val rdbuf_tag  = rvdffs_fpga(io.dma_axi_arid,rdbuf_en,dma_bus_clk,io.dma_bus_clk_en,clock)
  val rdbuf_sz   = rvdffs_fpga(io.dma_axi_arsize,rdbuf_en,dma_bus_clk,io.dma_bus_clk_en,clock)
 ...
 io.dma_axi_arready := ~(rdbuf_vld & ~rdbuf_cmd_sent)
  ...
  axi_mstr_sel     := Mux((wrbuf_vld & wrbuf_data_vld & rdbuf_vld), axi_mstr_priority, (wrbuf_vld & wrbuf_data_vld).asBool() )
