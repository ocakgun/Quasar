...
class ifu_aln_ctl extends Module with lib with RequireAsyncReset {
  ...
  io.ifu_i0_icaf_second := first4B & !icaf_eff(0) & icaf_eff(1)
  ...
  io.ifu_i0_instr := Mux1H(Seq((first4B & alignval(1)).asBool -> ifirst,
  ...
  if(BTB_ENABLE){
    ...
    io.i0_brp := 0.U.asTypeOf(io.i0_brp)
  }
